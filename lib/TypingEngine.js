...
module.exports = class TypingEngine {
    ...
    async typePage(url, site, browserPage) {
        ...
        if (pageType && gotoResponse.ok || url.startsWith("file://")) {
            ...
            // If the page type could not be determined, that is no error and a warning has already been logged.
    ...
    createPropertiesRecursively(parentNode, parentObjectType, parentObject, browserPage) {
        ...
        return propertyNames.map(async propertyName => {
            ...
            const selector = this.getEffectiveSelector(propertyDefinition, this.configuration.contentTypes);
            ...
            if (nodes && nodes.length > 0) {
                ...
                const propertyItemPromises = nodes.map(async node => {
                    ...
                    const [content, xPath, destination ] = await node.innerTextAndXPath();
                    const startSelector = new XPathSelector(xPath, 0);
                    const endSelector = new XPathSelector(xPath, content.length);
                    const selector = new RangeSelector(startSelector, endSelector);
                    let property;
                    if (Object.keys(propertyType.properties).length === 0) {
                        property = new Property(propertyDefinition.type, selector, content);
                    } else {
                        property = new Property(propertyDefinition.type, selector);
                    }
    ...
    createReferences(parentNode, parentObjectType, parentObject, browserPage) {
        ...
        return referencesNames.map(async referenceName => {
            ...
            const selector = this.getEffectiveSelector(referenceDefinition, this.configuration.referenceTypes);
            // TODO Adapt when ready: https://github.com/GoogleChrome/puppeteer/issues/508
            const nodes = await selector.evaluate(parentNode);
            ...
            if (nodes && nodes.length > 0) {
                ...
                const referenceItemPromises = nodes.map(async node => {
                    const referenceType = this.configuration.referenceTypes[referenceDefinition.type];
                    const [ innerText, xPath, destination ] = await node.innerTextAndXPath();
                    const startSelector = new XPathSelector(xPath, 0);
                    const endSelector = new XPathSelector(xPath, innerText.length);
                    const selector = new RangeSelector(startSelector, endSelector);
                    return new Reference(referenceDefinition.type, selector, destination);
                });
    ...
    async determinePageType(url) {
        ...
        logger.warn("No matching page type for %s.", url);
        return false;
    }
