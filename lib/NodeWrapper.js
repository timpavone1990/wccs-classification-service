...
module.exports = class NodeWrapper {
    ...
    async evaluate(action, ...args) {
        const evaluatePromise = this.node.evaluate ? this.node.evaluate(action, ...args) : this.browserPage.evaluate(action, ...args, this.node);
    ...
    async $$(selector) {
        return await this.evaluate((element, ...[selector, id, fakeNode]) => {
            try {
                ...
                const workingElement = fakeNode ? window.wcts.nodes[element.id][element.index] : element;
                ...
                nodes.forEach((node, index) => {
                    returnedNodes.push({
                        ...
                        "xPath": window.wcts.XPathCalculator.fromNode(workingElement)
                    });
                });
